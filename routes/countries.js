...
const { getCitiesByCountryId,getAll , byId } = require('../db/api/countries.js');
router.get('/',function(req,res,next){
    getAll()
        .then(countries=>{res.status(200).json({countries});console.log(countries)})
...
router.get('/:countryName/cities',function(req,res,next){
    getCitiesByCountryId(req.params.countryName)
